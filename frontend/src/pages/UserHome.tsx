...
const UserHome = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (user?.id) {
          ...
          const deliveriesResponse = await axios.get<Delivery[]>('http://localhost:8000/api/deliveries/all');
  ...
  const cancelOrder = async (orderId: string) => {
    try {
      const response = await axios.put(`http://localhost:8000/api/orders/cancel/${orderId}`);
